...
class AliengoEnv(gym.Env):
    ...
    def __init__(self, render=True):
        ...
        self.n_motors = 12
        self.state_space_dim = 12 * 2 + 7 # pos and vel for each motor, plus base position and orientation (quaternions)
        ...
        self.observation_space = spaces.Box(
            low=np.concatenate((self.actions_lb, -1e3 * np.ones(7))),
            high=np.concatenate((self.actions_ub, 1e3 * np.ones(7))),
            dtype=np.float32
            )
    ...
    def _reward_function(self) -> float:
        ...
        base_x_position = self.state[self.n_motors * 2]
        base_z_position = self.state[self.n_motors * 2 + 2]
    ...
    def _update_state(self):
        ...
        The state for the aliengo.urdf (from https://github.com/unitreerobotics/aliengo_pybullet) is the joint[0] position, joint[0] velocity, joint[1] position, ..., joint[11] velocity, then three values for global cartesian position of base, then four values for global orientation of base in quaternions.
