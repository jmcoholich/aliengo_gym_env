"""This file implements the gym environment of minitaur.
   The environmental parameters are added to the state. Intead of domain randomization, this env does 
   strategy optmization, taking the env params as inputs.
...
class MinitaurBulletEnv_Friction(gym.Env):
  ...
  def __init__(
    ...
    high = np.array([*self.minitaur.GetObservationUpperBound(), 
                    self._env_randomizer._minitaur_leg_mass_err_range[1],
                    self._env_randomizer._minitaur_leg_mass_err_range[1],
                    self._env_randomizer._minitaur_base_mass_err_range[1],
                    self._env_randomizer._motor_viscous_damping_range[1],
                    self._env_randomizer._battery_voltage_range[1],
                    self._env_randomizer._minitaur_leg_friction_range[1]])

    low = np.array([*self.minitaur.GetObservationLowerBound(), 
                    self._env_randomizer._minitaur_leg_mass_err_range[0],
                    self._env_randomizer._minitaur_leg_mass_err_range[0],
                    self._env_randomizer._minitaur_base_mass_err_range[0],
                    self._env_randomizer._motor_viscous_damping_range[0],
                    self._env_randomizer._battery_voltage_range[0],
                    self._env_randomizer._minitaur_leg_friction_range[0]])

    observation_high = (high + OBSERVATION_EPS)
    observation_low = (low - OBSERVATION_EPS)
  ...
  def render(self, mode="rgb_array", close=False):
    ...
    proj_matrix = self._pybullet_client.computeProjectionMatrixFOV(fov=60,
                                                                   ...
                                                                   nearVal=0.1,
                                                                   farVal=100.0)
    self._pybullet_client.addUserDebugText('mu: ' + str(self._env_randomizer.randomized_foot_friction),
                                          textPosition=(0,0,0) )
    (_, _, px, _,
  ...
  def _noisy_observation(self):
    ...
    return [*observation, *self._get_env_params()] 
  
  def _get_env_params(self):
    ''' Adds 6 environmental parameters to the state space ''' 

    leg_masses = self._env_randomizer.randomized_leg_masses
    base_mass = self._env_randomizer.randomized_base_mass
    motor_damping = self._env_randomizer.randomized_motor_damping
    battery_voltage = self._env_randomizer.randomized_battery_voltage
    foot_friction = self._env_randomizer.randomized_foot_friction
    return np.array([*leg_masses, base_mass, motor_damping, battery_voltage, foot_friction])
